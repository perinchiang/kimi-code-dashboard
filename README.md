...
[![Cross-platform tests](https://github.com/perinchiang/kimi-code-dashboard/actions/workflows/cross-platform-tests.yml/badge.svg)](https://github.com/perinchiang/kimi-code-dashboard/actions/workflows/cross-platform-tests.yml)
...
- 选项 4：按 Kimi Web 实例文件中的 PID 结束已启动的 Kimi Code Web 进程。
...
> **跨平台支持**：
> - 每次推送由 GitHub Actions 在 **Windows、macOS、Linux** 三套 runner 上执行 Python 编译、单元测试、Flask 路由加载、前端 JS 检查和安装脚本语法检查
> - **全平台可用**：数据可视化、会话与标题管理、产物与配置管理、Kimi Web 服务配置、主题切换、版本检查与一键更新、手动安装
> - **Windows / macOS**：支持开机自启（Windows Startup/Task Scheduler、macOS launchd）
